...
Param (
	[switch]$SkipTest,
	[string[]]$CommandPath,
	[string]$ModuleName = "PSPowerBITools",
...
if ($null -eq $CommandPath) {
	...
	$CommandPath = @((Join-PSFPath -Path "$testroot" "\..\" "functions" -Normalize), (Join-PSFPath -Path $testroot "\..\" "internal", "functions" -Normalize))
}

if (-not $ExceptionsFile -or ($null -eq $ExceptionsFile)) {
	#$ExceptionsFile = (Join-PSFPath -Path $global:testroot, "general" "Help.Exceptions.ps1" -Normalize)
	$ExceptionsFile = (Join-PSFPath -Path $testroot "general" "Help.Exceptions.ps1" -Normalize)
}
